...
if [ -z "$VERSION" ]; then
  ...
  echo "مثال: ./release.sh v3.5.0"
  exit 1
fi

if [[ ! "$VERSION" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  echo "❌ فرمت نسخه اشتباه است. باید مثل v3.5.0 باشد"
  exit 1
fi
...
# ═══ ۰. چک‌های قبل از انتشار ═══
echo "🔍 بررسی‌های قبل از انتشار..."
if ! node scripts/pre-release-check.mjs; then
  echo ""
  echo "❌ خطاهای بالا را رفع کن و دوباره تلاش کن."
  exit 1
fi
