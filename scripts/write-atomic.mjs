 ...
 *
 *   node scripts/write-atomic.mjs /workspace/.divan/og.jpg.tmp public/og.jpg
 *
 ...
 * /workspace/.divan/" contract rests on — cannot be provoked portably.
 */
    ...
    if (err?.code === "EXDEV") {
      throw new Error(
        `${staged} is on another filesystem than ${target}, so the hand-over cannot be a `
          + "rename — stage under /workspace/.divan/ instead",
      );
    }
    throw err;
  }
